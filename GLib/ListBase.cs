...
namespace GLib {
	...
	public abstract class ListBase : IDisposable, ICollection, GObject.IWrapper, ICloneable {
		...
		IntPtr AllocNativeElement (object element)
		{
			if (element_type == null) {
				...
				if (element_type == typeof (string))
					...
					return FileUtils.StringToFilenamePtr (element as string);
		...
		internal object DataMarshal (IntPtr data) 
		{
			...
			if (element_type != null) {
				if (element_type == typeof (string))
					...
					ret = FileUtils.FilenamePtrToString (data);
				else if (element_type == typeof (IntPtr))
					ret = data;
				else if (element_type.IsSubclassOf (typeof (GObject.Object)))
					ret = GObject.Object.GetObject (data, false);
				else if (element_type.IsSubclassOf (typeof (GObject.Opaque)))
					ret = GObject.Opaque.GetOpaque (data, element_type, elements_owned);
				else if (element_type == typeof (int))
					ret = (int) data;
				else if (element_type.IsValueType)
			...
			} else if (GObject.Object.IsObject (data))
				ret = GObject.Object.GetObject (data, false);
		...
		public void Empty ()
		{
			if (elements_owned)
				for (uint i = 0; i < Count; i++)
					if (typeof (GObject.Object).IsAssignableFrom (element_type))
						g_object_unref (NthData (i));
					else if (typeof (GObject.Opaque).IsAssignableFrom (element_type))
						GObject.Opaque.GetOpaque (NthData (i), element_type, true).Dispose ();
					else 
						g_free (NthData (i));
		...
		public static Array ListPtrToArray (IntPtr list_ptr, Type list_type, bool owned, bool elements_owned, Type elem_type)
		{
			...
			using (list)
				return list.ToArray (array_type);
		}
	}
}
