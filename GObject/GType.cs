// GObject.Type.cs - GObject GType class implementation
//
...
namespace GObject {
	...
	public struct GType {
		...
		static GType ()
		{
			...
			Register (GType.Object, typeof (GObject.Object));
			Register (GType.Pointer, typeof (IntPtr));
		...
		public static explicit operator GType (System.Type type)
		{
			...
			if (type.IsSubclassOf (typeof (GObject.Object))) {
				gtype = GObject.Object.LookupGType (type);
				Register (gtype, type);
				return gtype;
			}
			...
			if (pi != null)
				...
				gtype = (GType) pi.GetValue (null, null);
			} else if (type.IsSubclassOf (typeof (GObject.Opaque)))
				gtype = GType.Pointer;
			else
		...
		static string GetQualifiedName (string cname)
		{
			for (int i = 1; i < cname.Length; i++) {
				if (System.Char.IsUpper (cname[i])) {
					if (i == 1 && cname [0] == 'G')
						return "GObject." + cname.Substring (1);
					else
		...
		internal static GType LookupGObjectType (System.Type t)
		{
			...
			return GObject.Object.RegisterGType (t);
		}
