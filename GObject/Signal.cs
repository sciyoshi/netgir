// GObject.Signal.cs - signal marshaling class
//
...
namespace GObject {
	...
	public class Signal {
		...
		public static Signal Lookup (GObject.Object obj, string name)
		{
		...
		private static uint GetGQuarkFromString (string str) {
			IntPtr native_string = GObject.Marshaller.StringToPtrGStrdup (str);
			uint ret = g_quark_from_string (native_string);
			GObject.Marshaller.Free (native_string);
			return ret;
		}

		private static uint GetSignalId (string signal_name, GObject.Object obj)
		{
		...
		{
			IntPtr native_name = GObject.Marshaller.StringToPtrGStrdup (signal_name);
			uint signal_id = g_signal_lookup (native_name, typeid);
			GObject.Marshaller.Free (native_name);
			return signal_id;
		}
		
		public static ulong AddEmissionHook (string detailed_signal, GObject.GType type, EmissionHook handler_func)
		{
			uint gquark;
			string signal_name;
			ParseSignalDetail (detailed_signal, out signal_name, out gquark);
		...
		[DllImport("libgobject-2.0-0.dll")]
		static extern void g_signal_emitv (IntPtr instance_and_params, uint signal_id, uint gquark_detail, ref GObject.Value return_value);
