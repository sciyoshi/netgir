// GObject.Value.cs - GObject Value class implementation
//
...
namespace GObject {
	...
	public struct Value : IDisposable {
		...
		{
			IntPtr native_val = GObject.Marshaller.StringToPtrGStrdup (val);
			g_value_set_string (ref this, native_val); 
			GObject.Marshaller.Free (native_val);
		}
		...
		public Value (GObject.Object val) : this (val == null ? GType.Object : val.NativeType)
		{
			g_value_set_object (ref this, val == null ? IntPtr.Zero : val.Handle);
		}

		public Value (GObject.GInterfaceAdapter val) : this (val == null ? GType.Object : val.GType)
		{
			g_value_set_object (ref this, val == null ? IntPtr.Zero : val.Handle);
		}

		public Value (GObject.Object obj, string prop_name)
		{
			type = IntPtr.Zero;
			pad1 = new Padding ();
			pad2 = new Padding ();
		...
		public Value (IntPtr obj, string prop_name, Opaque val)
		{
			...
			pad1 = new Padding ();
			pad2 = new Padding ();
			InitForProperty (GObject.Object.GetObject (obj), prop_name);
			g_value_set_boxed (ref this, val.Handle);
		}

		public Value (string[] val) : this (new GObject.GType (g_strv_get_type ()))
		{
			if (val == null) {
			...
			for (int i = 0; i < val.Length; i++)
				Marshal.WriteIntPtr (native_array, i * IntPtr.Size, GObject.Marshaller.StringToPtrGStrdup (val[i]));
			Marshal.WriteIntPtr (native_array, val.Length * IntPtr.Size, IntPtr.Zero);

			g_value_set_boxed (ref this, native_array);

			for (int i = 0; i < val.Length; i++)
				GObject.Marshaller.Free (Marshal.ReadIntPtr (native_array, i * IntPtr.Size));
		...
		public static explicit operator string (Value val)
		{
			...
			return str == IntPtr.Zero ? null : GObject.Marshaller.Utf8PtrToString (str);
		}
		...
		public static explicit operator GObject.Opaque (Value val)
		{
			return GObject.Opaque.GetOpaque (g_value_get_boxed (ref val), (Type) new GType (val.type), false);
		}

		public static explicit operator GObject.Object (Value val)
		{
			return GObject.Object.GetObject (g_value_get_object (ref val), false);
		}

		[Obsolete ("Replaced by GObject.Object cast")]
		public static explicit operator GObject.UnwrappedObject (Value val)
		{
		...
		public static explicit operator string[] (Value val)
		{
			...
			for (int i = 0; i < count; i++)
				strings[i] = GObject.Marshaller.Utf8PtrToString (Marshal.ReadIntPtr (native_array, i * IntPtr.Size));
			return strings;
		}
		...
		object ToBoxed ()
		{
			...
			else if (t.IsSubclassOf (typeof (GObject.Opaque)))
				return (GObject.Opaque) this;
		...
		public object Val
		{
			...
			set {
				if (type == GType.Boolean.Val)
					...
					IntPtr native = GObject.Marshaller.StringToPtrGStrdup ((string)value);
					g_value_set_string (ref this, native);
					GObject.Marshaller.Free (native);
				} else if (type == GType.Pointer.Val) {
					...
					if(value is GObject.Object)
						g_value_set_object (ref this, (value as GObject.Object).Handle);
					else
						g_value_set_object (ref this, (value as GObject.GInterfaceAdapter).Handle);
				else if (GType.Is (type, GType.Boxed)) {
